...
console.log(a); // 10
```
上述代码，在**全局作用域**中， 且在**块级作用域** `{}` 中， a 输出的结果为 10， 由此可以看出 `var` 声明的变量不存在 块级作用域的概念
...
> 
> 2、let 定义的变量，有块的概念，不能跨块或者函数访问，不能变量提升，不可以重复声明
> 
> 3、const 定义的常量，使用必须初始化(赋值), 有块的概念，不能跨块或者函数访问，且不能修改值、变量提升、重复声明
> 
> 块级作用域是在 ES6 中新增的，在 ES6 之前只有全局作用域和函数作用域
> 
> 块级作用域 > {} 包括， if 语句和 for 语句里的 {} 也属于块作用域。 
> 
